...
class Maze {
  ...
  makeMaze() {
    ...
    let startX = Math.floor(Math.random() * this.w + 1);
    let startY = Math.floor(Math.random() * this.h + 1);
  ...
  randomDirection(x, y) {
    let dir = [];
    // x+1
    if(x + 1 <= this.w && !this.findCellByCoords(this.visited, x + 1, y)) {
      dir.push({ x: x + 1, y: y });
    }
    // x-1
  ...
  printCells() {
    ...
    for(let t = 0; t < this.cells.flat().length; t++) {
      output.push(this.cells.flat()[t].render());
    }
