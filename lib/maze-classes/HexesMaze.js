...
class HexesMaze extends Maze {
  ...
  printCells(showSolution = false, showDistances = false) {
    let output = ' ' + '    __  '.repeat(Math.floor(this.maxX / 2)) + '\n';
    ...
    for(let yIndex = this.maxY; yIndex >= 0; yIndex--) {
      ...
      for(let xIndex = 0; xIndex <= this.maxX + 1; xIndex++) {
        ...
        if(cellSW) {
          output += cellSW.toString(this.dimensions.width, this.start, this.end, showSolution, showDistances).topString;
        }
        else if(yIndex >= this.maxY && xIndex >= 1 && xIndex <= this.maxX) {
          output += (xIndex === 1) ? ' __ ' : '\\__ ';
        }
        output += (cellNE) ? cellNE.toString(this.dimensions.width, this.start, this.end, showSolution, showDistances).botString : '';
      }
      output += '\n';
    }
    return output;
  ...
  getRandomCell() {
    const randomX = Math.floor(Math.random() * this.dimensions.width) + 1;
    let randomY = Math.floor(Math.random() * this.dimensions.height) * 2 + 1;
    if(isEven(randomX)) randomY++;
