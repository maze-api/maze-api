  ...
  printCells(showSolution = true, showDistances = false) {
    ...
    for(let yIndex = this.maxY; yIndex >= 0; yIndex--) {
      ...
      for(let xIndex = 0; xIndex <= this.maxX + 1; xIndex++) {
        ...
        if(cellSW) {
          output += cellSW.toString(this.dimensions.width, showSolution, showDistances).topString;
        }
        else if(yIndex >= this.maxY && xIndex >= 1 && xIndex <= this.maxX) {
          output += (xIndex === 1) ? ' ▁▁ ' : '\\▁▁ ';
        }
        output += (cellNE) ? cellNE.toString(this.dimensions.width, showSolution, showDistances).botString : '';
      }
      output += '\n';
    }
    return output;
